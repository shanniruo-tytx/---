...
// 分批发送弹幕
function sendBarrageBatch(barrages, interval, maxLength) {
  var index = 0;
  var timer = setInterval(function () {
    if (index >= barrages.length) {
      clearInterval(timer);
      console.log("所有弹幕发送完成！");
      return;
    }
    var barrage = barrages[index].substring(0, maxLength);
    document.querySelector('.ChatSend-txt').value = barrage;
    document.querySelector('.ChatSend-button').click();
    console.log("发送弹幕: " + barrage);
    barrages[index] = barrages[index].substring(maxLength);
    if (barrages[index].length === 0) {
      index++;
    }
  }, interval);
}
...
// 创建前台说书元素
function createStorytellingElement() {
  var storytellingContainer = document.createElement('div');
  storytellingContainer.style.cssText = 'position: fixed; bottom: 10px; right: 10px; padding: 10px; background: #fff; border: 1px solid #000; z-index: 9999;';
  
  var titleLabel = document.createElement('p');
  titleLabel.innerHTML = '说书人脚本';
  ...
  maxLengthInput.placeholder = '每次发送的文本长度';
  maxLengthInput.style.border = '1px solid #000';
  maxLengthInput.value = 15;
  
  var intervalInput = document.createElement('input');
  intervalInput.type = 'number';
  intervalInput.placeholder = '发送间隔（毫秒）';
  intervalInput.style.border = '1px solid #000';
  intervalInput.value = 5000;

  var longTextInput = document.createElement('textarea');
  longTextInput.placeholder = '自定义长文本';
  longTextInput.style.border = '1px solid #000';
  
  storytellingContainer.appendChild(titleLabel);
  storytellingContainer.appendChild(document.createTextNode('文本长度：'));
  storytellingContainer.appendChild(maxLengthInput);
  storytellingContainer.appendChild(document.createTextNode('发送间隔：'));
  storytellingContainer.appendChild(intervalInput);
  storytellingContainer.appendChild(startButton);
  storytellingContainer.appendChild(stopButton);
  storytellingContainer.appendChild(longTextInput);
  
  document.body.appendChild(storytellingContainer);
...
var barrages = barragesData.split("\n");
console.log("弹幕加载完成！");

var storytellingElement = createStorytellingElement();

storytellingElement.startButton.addEventListener('click', function () {
  maxLength = parseInt(storytellingElement.maxLengthInput.value) || 15;
  intervalTime = parseInt(storytellingElement.intervalInput.value) || 5000;
  console.log("开始发送弹幕...");
  sendBarrageBatch(barrages, intervalTime, maxLength);
});

storytellingElement.stopButton.addEventListener('click', function () {
  console.log("停止发送弹幕！");
  clearInterval(timer);
});
